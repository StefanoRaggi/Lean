...
namespace QuantConnect.Tests.Common.Orders.Fills
{
    ...
    public class ImmediateFillModelTests
    {
        ...
        public void MarketOrderFillsAtBidAsk(OrderDirection direction)
        {
            ...
            var security = new Forex(exchangeHours, quoteCash, config, symbolProperties, new CashBookCurrencyConverter(new CashBook()));
        ...
        public void ImmediateFillModelUsesPriceForTicksWhenBidAskSpreadsAreNotAvailable()
        {
            ...
            var security = new Security(SecurityExchangeHoursTests.CreateUsEquitySecurityExchangeHours(), config, new Cash(CashBook.AccountCurrency, 0, 1m), SymbolProperties.GetDefault(CashBook.AccountCurrency), new CashBookCurrencyConverter(new CashBook()));
            security.SetLocalTimeKeeper(timeKeeper.GetLocalTimeKeeper(TimeZones.NewYork));
