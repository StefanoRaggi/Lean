...
namespace QuantConnect.Tests.Engine
{
    ...
    public class BasicOptionAssignmentSimulationTests
    {
        ...
        public void GenerateSimulationDatesFromOptionExpirations()
        {
            ...
            securities.Add(option1, new Option(SecurityExchangeHours, CreateTradeBarDataConfig(SecurityType.Equity, option1), new Cash(CashBook.AccountCurrency, 0, 1m), new OptionSymbolProperties(SymbolProperties.GetDefault(CashBook.AccountCurrency)), new CashBookCurrencyConverter(new CashBook())));

            var option2 = Symbol.CreateOption("SPY", Market.USA, OptionStyle.American, OptionRight.Call, 193m, new DateTime(2016, 02, 19));
            securities.Add(option2, new Option(SecurityExchangeHours, CreateTradeBarDataConfig(SecurityType.Equity, option2), new Cash(CashBook.AccountCurrency, 0, 1m), new OptionSymbolProperties(SymbolProperties.GetDefault(CashBook.AccountCurrency)), new CashBookCurrencyConverter(new CashBook())));

            var option3 = Symbol.CreateOption("SPY", Market.USA, OptionStyle.American, OptionRight.Call, 190m, new DateTime(2016, 03, 18));
            securities.Add(option3, new Option(SecurityExchangeHours, CreateTradeBarDataConfig(SecurityType.Equity, option3), new Cash(CashBook.AccountCurrency, 0, 1m), new OptionSymbolProperties(SymbolProperties.GetDefault(CashBook.AccountCurrency)), new CashBookCurrencyConverter(new CashBook())));
            ...
            Func<OptionRight, decimal, decimal, decimal, Option> optionDef =
                (right, strikePrice, bidPrice, askPrice) =>
                {
                    ...
                    var option = new Option(SecurityExchangeHours, CreateTradeBarDataConfig(SecurityType.Option, symbol),
                        new Cash(CashBook.AccountCurrency, 0, 1m), new OptionSymbolProperties(SymbolProperties.GetDefault(CashBook.AccountCurrency)), new CashBookCurrencyConverter(new CashBook()));
